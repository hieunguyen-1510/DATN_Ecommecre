...
export const getProductInfo = async (productName) => {
    ...
    if (products.length > 0) {
        return products.map(p => `Tên: ${p.name}, Giá: ${p.price}, Mô tả: ${p.description}`).join('\n');
    }
    return null;
}
...
export const getOderStatus = async (orderId, userId) => {
    ...
    if (order) {
        return `Đơn hàng #${orderId.slice(-6)} của bạn đang có trạng thái: ${orderId.status}.`;
    }
    return null;
}
...
export const buildKnowledgeBaseString = async (userMessage, userId = null) => {
    ...
    if (userId && (userMessage.toLowerCase().includes('đơn hàng') || userMessage.toLowerCase().includes('trạng thái đơn hàng'))) {
        ...
        if (orderIdMatch) {
            ...
            if (orderStatus) {
                ...
                kb += "--- Không tìm thấy đơn hàng với mã này hoặc không thuộc về bạn. ---\n\n";
            }
        }
    }
    return kb;
