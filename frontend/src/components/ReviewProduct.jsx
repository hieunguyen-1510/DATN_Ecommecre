...
const ReviewProduct = ({ productId }) => {
  ...
  useEffect(() => {
    ...
    const average = total > 0 
      ? (ratings.reduce((acc, curr, index) => acc + (curr * (index + 1)), 0) / total).toFixed(1)
      : 0;
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await fetch(`${backendUrl}/api/reviews?productId=${productId}`);
  ...
  return (
    <div className="mt-12 bg-white p-6 rounded-lg shadow-md max-w-4xl mx-auto">
        ...
        <div className="text-center">
          ...
          <div className="text-yellow-500 text-2xl mb-2">
            {[...Array(5)].map((_, i) => (
              <span
                key={i}
                className={i < Math.floor(totalRating) ? 'text-yellow-500' : 'text-gray-300'}
              >
                ★
              </span>
            ))}
      ...
      <div className="border-t pt-6">
        <h2 className="text-xl font-bold text-gray-800 mb-4">Viết đánh giá của bạn</h2>
        ...
        <button
          ...
          className="bg-red-600 text-white px-8 py-3 rounded-lg font-medium hover:bg-red-700 transition-colors float-right"
        >
          Gửi đánh giá
        </button>
      </div>
      ...
      <div className="mt-12 space-y-6">
        ...
        {filteredReviews.length === 0 ? (
          <div className="text-gray-500 text-center py-8">Chưa có đánh giá phù hợp</div>
        ) : (
