...
const CancelConfirmationModal = ({ isOpen, onClose, onConfirm, orderId }) => {
  ...
  // Variants for overlay and modal
  ...
  // Handle confirm click, passing the reason
  const handleConfirm = () => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div
            className="bg-white p-8 rounded-2xl shadow-2xl max-w-sm w-full relative transform scale-100"
          ...
          >
            ...
            <p className="text-gray-700 text-center mb-4 leading-relaxed">
              Bạn có chắc chắn muốn hủy đơn hàng <span className="font-semibold text-orange-600">{orderId?.substring(0, 8).toUpperCase()}</span> này không?
              <br />
